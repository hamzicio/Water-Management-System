...
public:
    ...
    Order(unsigned long b, const char* c,const char* d,const char* e,unsigned int f,int g=0,const char* h=NULL,const char* i=NULL):l(b,c,d,e,f,g,h,i)
...
#include "File.txt"
    ...
    MYSQL_ROW row=l.getrow();
        ...
        string user_name=l.getUsername();

        string id_query="SELECT * FROM customers WHERE username='"+user_name+"'";
        const char *q_id=id_query.c_str();

        mysql_query(conn, q_id);

        res=mysql_store_result(conn);

        char *id_c=row[0];
        string id=string(id_c);

        input_ID=id;

        string refill,empty_rec,total,cash,credit,deposit;

        mysql_query(conn, "SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='cp_project' AND TABLE_NAME='orders'");
       ...
       deposit="0";
        ...
        if(rows_fetched>0){
            ...
            string final_input="insert into orders(ID,Name,user_id,Date,Empty,Refill,Cash,Credit,Bottle_Balance,Balance,Total,Deposit) values ('"+entries_id+"','"+name_of_cust+"','"+input_ID+"','"+new_date+"','"+empty_rec+"','"+refill+"','"+cash+"','"+credit+"','"+final_bottle_balance+"','"+final_balance+"','"+total+"','"+dep_ins+"')";
...
int main()
{
    Order obj(0,"cp_project","182.50.132.112","aliakber123",0,0,"aliakber",0);
