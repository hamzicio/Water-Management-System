...
public:
    ...
    virtual void show_current_record()=0;

};

class Current_Record:public Record{
    Login l;
...
public:
  ...
  //  int qstate;  // to be removed.

       // string input_id;  // to be removed
        ...
        char *user_id_c=row[0];
         user_id=string(user_id_c);
        char *deposit_c=row[5];
        deposit=string(deposit_c);
        char *email_c=row[9];
        email=string(email_c);
        char *contact_c=row[3];
        contact=string(contact_c);
        char *date_from_c=row[1];
        date_from=string(date_from_c); // to be removed
...
class Old_Record:public Record{
    Login l;

public:
    Old_Record(unsigned long b, const char* c,const char* d,const char* e,unsigned int f,int g=0,const char* h=NULL,const char* i=NULL):l(b,c,d,e,f,g,h,i)
    ...
    void show_current_record(){
        ...
        contact=string(contact_c);
        char *date_from_c=row[1];
         date_from=string(date_from_c);


        string rec_query="SELECT * FROM old_entries WHERE user_id='"+user_id+"'";
        ...
        if(row_req>0){
            cout<<endl<<"Your Old Record/Bill is as follows:"<<endl<<endl;
...
int main(){
    ...
    cout<<"For Current Record, press 1 and for Old Record, press 2"<<endl;
    ...
    do{
        ...
        if(input=="1"){
            ptr=new Current_Record(0,"cp_project","182.50.132.112","aliakber123",0,0,"aliakber",0);
            check=true;
        }else if(input=="2"){
            ptr=new Old_Record(0,"cp_project","182.50.132.112","aliakber123",0,0,"aliakber",0);
            check=true;
        }
